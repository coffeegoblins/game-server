require(['src/pathManager', 'src/map'], function (PathManager, Map)
{
    ...
    PathManagerTest.prototype.testDiagonalMovementNearWorldObjects = function ()
    {
        ...
        var pathNodes = PathManager.calculatePath(map, unit, 2, 2);

        assertEquals('Path was not the correct length', 2, pathNodes.length);
        assertTrue('First path node was incorrect', pathNodes[0].x === 2, pathNodes[0].y === 1);
        assertTrue('Second path node was incorrect', pathNodes[1].x === 2, pathNodes[1].y === 2);
    };
    ...
    PathManagerTest.prototype.testDiagonalMovementNearClimbableWorldObjects = function ()
    {
        ...
        assertTrue('Path node was incorrect', pathNodes[0].x === 2, pathNodes[0].y === 2);
    };
});